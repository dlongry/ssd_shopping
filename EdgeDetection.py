...
class EdgeDetection:
    ...
    def get_ori_place(self,tclass,tbbox):
        k = self.context.redis_connection_intermediate_queue.keys()
        if len(k) < 5:
            print("EdgeDetection: len of intermediate_queue is 0")
        else:
            for results in self.context.redis_connection_intermediate_queue.mget(k[:-5]):
                try:
                    d = json.loads(results)
                except:
                    print("get_ori_place ,json.load error")
                    pass
                nclass = d['rclasses']
                nsore = d['rscores']
                nbbox= d['rbboxes']

                if tclass not in nclass:
                    continue
                else:
                    print("ori_class:", tclass)
                    return 1
        return 0
    ...
    def edge_detector(self, img):
        ...
        for i in range(rclasses.shape[0]):
            ...
            if ymin < border_ymax and (rclasses[i] == 1 or rclasses[i] == 2): # only for oreo and TesDi
                ...
                ori_place = self.get_ori_place(rclasses[i], rbboxes[i])
                ori_places.append(ori_place)

        return ori_places, tclasses, tscores, tbboxes
